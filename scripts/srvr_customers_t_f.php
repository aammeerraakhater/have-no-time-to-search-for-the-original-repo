...
$city = 'طوخ';
$servicesType = 'فلتر';
$sql = "SELECT customerID , usrName, phone, delegate, city, address, servicesType FROM customer  WHERE city='$city' AND servicesType LIKE'%$servicesType%'";

if (isset($_GET['search']) && !empty($_GET['search']['value'])) {
    $search = $_GET['search']['value'];
    $sql .= sprintf(" AND (usrName like '%s' OR phone like '%s' OR city like '%s' OR address like '%s')", '%'.$con->real_escape_string($search).'%', '%'.$con->real_escape_string($search).'%', '%'.$con->real_escape_string($search).'%', '%'.$con->real_escape_string($search).'%');
}
