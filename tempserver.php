...
if (isset($_GET['city']) && isset($_GET['servicesType'])) {
    ...
    $sql .= sprintf(" WHERE city = '%s' AND servicesType = '%s'", '%'.$con->real_escape_string($city).'%', '%'.$con->real_escape_string($servicesType).'%');
    if (isset($_GET['search']) && !empty($_GET['search']['value'])) {
        $search = $_GET['search']['value'];
        $sql .= sprintf(" AND usrName like '%s' OR phone like '%s'", '%'.$con->real_escape_string($search).'%', '%'.$con->real_escape_string($search).'%');
    }
    
}elseif (isset($_GET['search']) && !empty($_GET['search']['value'])) {
    $search = $_GET['search']['value'];
